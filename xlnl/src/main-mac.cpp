...
* へ手紙を書いてください。
* 
* $Id: main-mac.cpp,v 1.3 2014/01/07 23:55:39 bowkenken Exp $
***************************************************************/
...
void	init_mac( int argc, char **argv, NSWindow *window )
{
	...
	if( g_flg_gui ){
		init_mac_gui( argc, argv );
	} else if( g_flg_cui_mouse ){
		init_mac_cui_mouse( argc, argv );
	} else if( g_flg_cui ){
		init_mac_cui( argc, argv );
	}
}
...
- (void)handleTimerDrawMap:(NSTimer *)timer
{
	...
	if( get_scene() == SCENE_N_LAST_BOSS ){
		...
		if( gPcgDun.getFlgUpdateRequest() ){
			if( ![gMainMapView needsDisplay] ){
				gPcgDun.setFlgUpdateRequest( false );
